    ...
    for schema_path in schemas:
        ...
        try:
            ...
            if result and len(result) > 0 and result[0].get('total'):
                ...
                logger.debug(f"[TRINO] Improvements found with schema: {schema_path}")
                return improvements
        except Exception as e:
            logger.debug(f"[TRINO] Schema {schema_path} failed: {str(e)[:80]}")
            continue

    logger.debug(f"[TRINO] Could not find improvements for {provider_name} - using defaults")
    return improvements
